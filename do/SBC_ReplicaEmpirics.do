...
set more off
set matsize 1000
cd "/Users/sergiosalgado/Dropbox/FIRM_SKEWNESS_205/Data/PlotsSep2018/ShareData/"	// where the data is
...
*##################################	
	...
	*NOTE: data from the US is the same as the one used in column 3
	...
	sort iso3 qtr
	merge 1:1 iso3 qtr using "replicationxls/auxCQ.dta",nogenerate
	split(qtr), p(q)
	destring qtr1 qtr2, replace 
	rename qtr1 year
	rename qtr2 quarter
...
*Replica Figure 2A:
	...
	*Recession variable 
	...
	replace recession = 1 if year == 2000
...
*Replica Figure 3A:
	...
	*Plot
	...
	ytitle("P9050 and P5010 fo Employment Growth (%)", axis(2)) ylabel(,axis(2))) , ///
...
*Replica Figure 4B1 and 4B2
	...
	*Saving Scatter 
	...
	xtitle("Average of Firm Employment Growth (%)")  plotregion(lcolor(black)) ///
	...
	*Saving Scatter
	tw (scatter ksk me, mcolor(navy) msize(large) msymbol(O) mfcolor(navy*0.25)) ///
	(lfit ksk me,  lpattern(dash) lwidth(thick)) ,  ///						
	xtitle("Average of Firm Sales Growth (%)")  plotregion(lcolor(black)) ///
	...
	*in the paper.
	import excel using "replicationxls/SBC_USA_AND_CROSSCOUNTRY.xls", sheet("USA Employment Scatter") cellrange(A1:B51) first clear 
	replace ksk = 100*ksk
	replace me = 100*me
...
*Replica Figure 6
	...
	*Plot
	...
	(connected  ksk year if group == "all", color(black) lpattern(solid) ///
		msymbol(S) mfcolor(black*0.25) yaxis(2) ), ///
		ytitle("Kelley Skewness of Employment Growth (%)", axis(2)) ylabel(-30(10)20,axis(2)) ///
	 graphregion(color(white)) xtitle("") xlabel(1978(4)2014,grid)  plotregion(lcolor(black)) ///
...
*Replica Figure 7
	...
	*Figure 7A
	...
	(connected  ksk ksk2 ksk3 year  , color(blue black red) mfcolor(blue*0.25 black*0.25 red*0.25) lpattern(solid solid solid) msymbol(O S D) ///
		 yaxis(2) yscale(alt axis(2)) ///
	ytitle("Kelley Skewness of Employment Growth (%)", axis(2)) ylabel(,axis(2))) , ///
	...
	cap noisily: graph export "figs/SBC_Fig7A.pdf", replace	
	...
	*Figure 7B
	...
	order(2 "KSK of Log-change" 3 "KSK of Arc-percent change"))	
		cap noisily: graph export "figs/SBC_Fig7B.pdf", replace	
...
*Replica Appendix figure 12A, 12B, and 12C
	...
	*Figure 12C
	...
	legend(position(7) ring(0) rows(2) order(2 "All" 3 "Young" 4 "Middle" 5 "Mature") ///
		region(lcolor(white))) graphregion(color(white))
	cap noisily: graph export "figs/SBC_Fig12C.pdf", replace	
